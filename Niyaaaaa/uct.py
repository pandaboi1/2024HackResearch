...
class UCT(object):
    ...
    def get_action(self):
        ...
        print(f"UCT Legal actions: {legal}")
        ...
        # Return the action with the highest average value.
            # Return the action with the highest average value among legal moves
        legal_actions = set(legal)
        sorted_actions = sorted(
            (a for a in self.data['actions'] if a['action'] in legal_actions),
            key=lambda x: (x['percent'], x['plays']),
            reverse=True
        )
        
        if not sorted_actions:
            # If no legal actions are found, choose a random legal move
            chosen_action = choice(legal)
        else:
            chosen_action = sorted_actions[0]['action']

        return {
            'type': 'action',
            'message': self.board.to_json_action(chosen_action),
            'extras': self.data.copy(),
        }
